...
"""Prompt construction for lane-center CoT consistency analysis.

This prompt evaluates whether the action stated in a driving model's
chain-of-thought matches the trajectory it produced. It expects trajectory
features from the ``center_of_lane`` consistency variant: a dual view with an
ego-frame trajectory and a route-consistent lane-relative reference built from
the map graph.

The lane-relative view reports longitudinal progress, signed lateral offset,
offset change, lateral velocity, acceleration, and heading change. Offset
signals are used for lane keeping, nudges, drifts, and lane changes. Heading
signals are used for turns, because a correctly executed turn can stay centered
on a turning lane route while still rotating the vehicle by tens of degrees.
The ego-frame view is included as an absolute displacement sanity check.
...
def build_prompt(cot_text: str, traj_features: dict) -> str:
    """Build the lane-center CoT consistency evaluation prompt.
