...
Builds the multimodal evaluation prompt sent to Gemini.
Single-dimension rubric (mirrors the alpamayo gemini_inference.py):
...
def build_prompt(cot_text: str, traj_features: dict) -> str:
    ...
    if reference_frame == "lane_center":
        ...
        trajectory_title = (
            "Predicted Trajectory (lane-relative: sampled deltas to lane center):"
        )
        coordinate_frame = """=== COORDINATE FRAME (LANE-RELATIVE) ===
- s_lane: longitudinal progress along the lane centerline (meters).
- offset_m: signed lateral offset from lane centerline. Positive = left of lane center, negative = right.
- delta_offset_m: offset_m relative to t=0. Positive means moved left relative to starting lane position.
- lat_vel: lateral velocity with respect to lane center (positive left, negative right).
- accel: longitudinal acceleration proxy along lane progress (positive speed-up, negative slow-down).

Important: For curved roads, do NOT judge lateral intent using global/world turning direction alone.
